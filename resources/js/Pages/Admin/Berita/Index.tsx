...
export default function Index({ berita }: { berita: NewsAnnouncementsAchievements[] }) {
    return (
        <AdminLayout title="Kelola Berita">
            ...
            >
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-2xl font-bold text-slate-800">Daftar Berita, Pengumuman, dan Prestasi</h1>
                    <Link
                    ...
                    <table className="min-w-full bg-white rounded-lg shadow overflow-hidden">
                        <thead className="bg-sky-50">
                            <tr>
                                ...
                                <th className="py-3 px-6 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Tanggal</th>
                            ...
                            {berita.map((item, index) => (
                                ...
                                >
                                    ...
                                    <td className="py-4 px-6 whitespace-nowrap text-right">
                                        <Link href={route('admin.berita.edit', item.id)} className="text-slate-500 hover:text-sky-600 transition mr-4">
                                            <Edit size={18} />
                                        </Link>
                                        <Link href={route('admin.berita.destroy', item.id)} method="delete" as="button" className="text-slate-500 hover:text-red-600 transition">
                                            <Trash2 size={18} />
                                        </Link>
                                    </td>
                                </motion.tr>
